...
def send_record():
    ...
    for row in csvreader:
        key = {"Date": str(row[0])}
        value = {"Date": str(row[0]), "Open": float(row[1]), "High": float(row[2]), "Low": float(row[3]), "Close": float(row[4]), "Volume": str(row[5]), "Market Cap": str(row[6])}

        try:
            producer.produce(topic='bitcoin_price_training', key=key, value=value)
